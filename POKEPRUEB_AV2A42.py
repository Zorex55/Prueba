...
#Efectos de sonido
def Reproducir_Impactrueno(file):
 sound_effect = pg.mixer.Sound(file)
 sound_effect.play()

def Reproducir_Cola(file):
     sound_effect = pg.mixer.Sound(file)
     sound_effect.play()

def Reproducir_Slam(file):
    sound_effect = pg.mixer.Sound(file)
    sound_effect.play()

def Reproducir_Electrobola(file):
    sound_effect = pg.mixer.Sound(file)
    sound_effect.play()

def Reproducir_GolpeRoca(file):
    sound_effect = pg.mixer.Sound(file)
    sound_effect.play()
    
    
#Música de fondo (usa Pygame), comentado hasta que nos den luz verde para usar Pygame
ruta_musica = r"C:\Users\unity\Downloads\Regirock, Regice,  Registeel Battle - Pokémon Ruby  Sapphire [High Quality]-YoutubeConvert.cc.wav"
...
DigHP2 = tk.Label(juego, text = '120', font = Digfont)
...
Caja = Image.open('C:/Users/unity/OneDrive/Imágenes/Imagenes Poke/DS DSi - Pokemon Platinum - Text Box Styles.png')
...
#Vida de Pikachu
Digfont = tF.Font(family = "Helvetica", size = 12)
PikHP = tk.Label(juego, textvariable = pikachu_HP , font = Digfont)
PikHP.place(x = 265, y = 250)
Barra = tk.Label(juego, text = "/")
Barra.place(x = 310, y =250)
PikHP2 = tk.Label(juego, text = '150', font = Digfont)
...
GOP = tk.Label(juego, text = '', font = Digfont)
...
contador5 = tk.Label(juego, text = "Res: 3")
contador5.place(x = 450, y = 375)
...
vida = tk.Button(juego, text = "Usar Poción", command = VidaS)
vida.place(x = 370, y = 375)
